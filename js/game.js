// Pure rules engine for Antok Phrases.
...
// same reason: never read the clock internally, so the engine stays
// unit-testable with fake time.
...
// same principle as timed-wordy's explodeIfDue. If the deadline has passed,
// auto-skips to the next puzzle (no score change) and leaves its timer
// paused, waiting for the Host to start it again. Returns true if a skip
// just happened (the caller should re-render and re-broadcast).
...
// Display — this is the one place that decides what a blank tile looks like.
...
// auto-validate a typed guess instead of trusting a self-judged "Got It" tap.
// Answers here are always plain A-Z + single spaces (see categories.js), so
// no punctuation stripping is needed — just fold case and collapse
// whitespace from copy/paste or a stray double space.
