...
// keeps the full answer locally, while the Display only receives revealed
// answer letters through maskedAnswer().
function renderPhrase(container, prompt) {
...
const CONFETTI_COLORS = ['#ff3d7f', '#22d3ee', '#ffd23f', '#7dffb3'];
...
// from their own clock — see plan.md's networking model; nothing is pushed
// over the wire every tick) ----------
...
function renderHostPanel() {
  ...
  renderPhrase($('host-phrase'), puzzle?.prompt);
...
// The Host's clock is the only authority on timer expiry (same principle as
// timed-wordy's explodeIfDue). Both roles otherwise just repaint their own
// countdown digits every tick from their own clock — nothing is pushed over
// the network except when the game state actually changes (timer started,
...
function renderSinglePanel() {
  ...
  renderPhrase($('single-phrase'), puzzle?.prompt);
...
function resetDisplayView() {
  ...
  $('display-phrase').classList.remove('phrase-blurred');
...
function handleDisplayState(state, hostNow) {
  ...
  if (state.phase === PHASE.PLAYING && state.puzzle) {
    ...
    renderPhrase($('display-phrase'), state.puzzle.prompt);
    renderTiles($('display-tiles'), state.puzzle.masked);
    // Blur the phrase clue until the Host actually starts the timer for this
    ...
    // so the phrase shows immediately.
    const phraseBlocked = state.timerSeconds && state.timerStatus === TIMER_STATUS.PAUSED;
    $('display-phrase').classList.toggle('phrase-blurred', phraseBlocked);
